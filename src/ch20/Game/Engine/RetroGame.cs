...
public abstract class RetroGame : Game
{
    ...
    protected void RelayoutForScreenSize(bool force = false)
    {
        ...
        Pad.Layout(w, h, landscape: w >= h, rotated: Screen.Rotated);
    ...
    /// <summary>
    /// Whether the picture is currently turned. Held here rather than only on
    /// the screen because <see cref="SetVirtualSize"/> builds a replacement
    /// screen, and the new one has to inherit the quarter turn.
    /// </summary>
    ...
    {
        _rotated = mode != OrientationMode.Sensor && mode != PrimaryOrientation;
        if (Screen == null) return;
        Screen.Rotated = _rotated;
        RelayoutForScreenSize(force: true);
    }
    ...
    public void SetVirtualSize(int width, int height)
    {
        ...
        Screen?.Dispose();
        Screen = new VirtualScreen(GraphicsDevice, width, height) { Rotated = _rotated };
    ...
    {
        // Losing focus on a phone means a call or the app switcher. Stop loops
        // immediately or the barrel rumble carries on over the home screen.
        Audio?.StopAllLoops();
